...
data Val2 where
    ...
    ValFun2 :: EvalEnv2 -> Var -> Stx -> Val2
    ValCode2 :: Stx -> Val2

instance Show Val2 where 
    show (ValConst2 c) = "<const '" ++ show c ++ "'>"
    show (ValFun2 closureVars name body) = "<cls: " ++ "\\" ++ name ++ ". " ++ show body ++ ">"
...
evalEnv2 :: HasCallStack => EvalM2 EvalEnv2
evalEnv2 = EvalM2 $ \e -> (Right e, e)
...
val2ToValFun2 (ValFun2 closureArgs xname body) =  do
    pure $ \xval -> scopedEnv2 $ do 
        forM (M.toList closureArgs) (uncurry evalInsertEnv2)
        evalInsertEnv2 xname xval
        eval2 body
...
eval2 (StxLam D xname body) = do
  env <- evalEnv2
  pure $ ValFun2 env xname body
eval2 (StxFix D e) = eval2 e >>= todo "Unsure how to implement fix" 
-- Eval2 (StxFix D e) = eval2 e >>= todo "Unsure how to implement fix" $ \case
--   ValFun2 env x body -> undefined -- f (ValFun2 f) -- this only works in lazy language.
